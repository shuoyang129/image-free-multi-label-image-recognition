...
def mAP(targs, preds, datasetname=None, f=True):
    ...
    if datasetname == "voc2007" and f==False:
        return 100 * ap.sum() /20, ap
...
def save_checkpoint(state, is_best, filepath='', prefix=None):
    ...
    if is_best:
        if prefix is None:
            ...
            shutil.copyfile(
                os.path.join(filepath, 'checkpoint.pth.tar'),
                os.path.join(filepath, '%s_model_best.pth.tar' % prefix))
