...
class AttentionConv(nn.Module):
    def __init__(self, embed_dim: int, spec_dim: int, output_dim: int = None):
        ...
        self.positional_embedding = nn.Parameter(
            torch.randn(spec_dim**2 + 1, embed_dim) / embed_dim**0.5)
        self.k_proj = nn.Linear(embed_dim, embed_dim)
        self.q_proj = nn.Linear(embed_dim, embed_dim)
        self.v_proj = nn.Linear(embed_dim, embed_dim)
        self.c_proj = nn.Linear(embed_dim, output_dim or embed_dim)
    ...
    def forward(self, x):
        ...
        if h * w + 1 != self.positional_embedding.shape[0]:
            w_spacial = self.positional_embedding[1:].permute(1, 0).reshape(
                1, -1, self.spec_dim, self.spec_dim)
            w_spacial = F.interpolate(w_spacial, size=(h, w), mode='bicubic')
            w_special = w_spacial.reshape(-1, h * w).permute(1, 0)
            positional_embedding = torch.cat(
                [self.positional_embedding[:1], w_special], dim=0)
        else:
...
class ModifiedResNet_conv_proj(nn.Module):
    ...
    def __init__(self,
        ...
        self.embed_dim = width * 32  # the ResNet feature dimension
        self.attnpool = AttentionConv(self.embed_dim, input_resolution // 32,
                                      output_dim)
...
def build_model_conv_proj(state_dict: dict, cfg):
    ...
    model = CLIP_conv_proj(embed_dim, cfg.INPUT.SIZE[0], vision_layers,
    ...
    for k, v in old_state_dict.items():
        if k in state_dict.keys():
            if 'visual' in k and ('v_proj' in k or 'c_proj' in k):
                ...
                w_spacial = state_dict[k][1:].permute(1,
                                                      0).reshape(1, -1, 7, 7)
                new_size = int((old_state_dict[k].shape[0] - 1)**0.5)
                w_spacial = F.interpolate(w_spacial,
