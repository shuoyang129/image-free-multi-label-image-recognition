...
def build_model(cfg, args, classnames):
    ...
    model = eval(args.model_name)(cfg, classnames)
    network_name = model.network_name if hasattr(
        model, 'network_name') else cfg.MODEL.BACKBONE.NAME
    arch_name = "{dataset}-{model_name}-{arch_name}".format(
        dataset=cfg.DATASET.NAME,
        model_name=args.model_name,
        arch_name=network_name)
    # add setting info only in training

    arch_name += "{}".format('-' + args.prefix if args.prefix else "")
    if not args.evaluate:
        arch_name += "-{}-bs{}-e{}".format(cfg.OPTIM.LR_SCHEDULER,
